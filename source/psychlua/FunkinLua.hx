...
#if (LUA_ALLOWED && !lumod)
typedef State = cpp.RawPointer<Lua_State>;
#elseif LUA_ALLOWED
typedef State = lumod.State;
#end
...
class FunkinLua {
    ...
    public function new(scriptName:String, ?skipPsychStuff:Bool = false) {
        ...
        try {
            var isString:Bool = !sys.FileSystem.exists(scriptName);
            var status:Int = 0;
            if (!isString)
                status = #if MODS_ALLOWED sys.FileSystem.exists(scriptName) ? LuaL.dofile(lua, scriptName) : #end LuaL.dostring(lua, sys.io.File.getContent(scriptName));
            else
            ...
            if (status != 0) {
                ...
                if (ClientPrefs.isDebug()) {
                    Sys.println('Lua (${scriptName}): ' + errorMsg);
                    #if desktop
        ...
        if (ClientPrefs.isDebug())
            Sys.println('Lua file loaded succesfully: ' + haxe.io.Path.join([Sys.getCwd(), scriptName]) + ' (${Std.int(Date.now().getTime() - times)}ms)');
    ...
    function findScript(scriptFile:String, ext:String = '.lua')
    {
        ...
        if(sys.FileSystem.exists(preloadPath))
        #else
    ...
    public function getErrorMessage(status:Int):String {
        ...
        if (v == null || v == "") {
            switch(status) {
                case type if (type == Lua.ERRRUN): return "Runtime Error";
                case type if (type == Lua.ERRMEM): return "Memory Allocation Error";
                case type if (type == Lua.ERRERR): return "Critical Error";
            }
    ...
    public function addLocalCallback(name:String, myFunction:Dynamic)
    {
        ...
        Convert.addCallback(lua, name, null);
        #end
    }

    public function addLuaCallback(name:String, myFunction:Dynamic) {
        #if LUA_ALLOWED
        Convert.addCallback(lua, name, myFunction);
        #end
    }
    ...
    public function initLuaShader(name:String)
    {
        ...
        for (folder in foldersToCheck)
        {
            if(sys.FileSystem.exists(folder))
            {
                ...
                if(sys.FileSystem.exists(frag))
                {
                    frag = sys.io.File.getContent(frag);
                    found = true;
                }
                else frag = null;

                if(sys.FileSystem.exists(vert))
                {
                    vert = sys.io.File.getContent(vert);
                    found = true;
                }
                else vert = null;
