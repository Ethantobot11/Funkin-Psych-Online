...
private class BlurShader extends BitmapFilterShader
{
    @:glFragmentSource("uniform sampler2D openfl_Texture;
        ...
        varying vec2 vRadiusTexel;

        void main(void) {
            vec4 sum = texture2D(openfl_Texture, vTexCoord) * 0.39894;
            
            vec2 step1 = vRadiusTexel * 0.5;
            vec2 step2 = vRadiusTexel * 0.75;

            sum += texture2D(openfl_Texture, vTexCoord - step1) * 0.24197;
            sum += texture2D(openfl_Texture, vTexCoord + step1) * 0.24197;
            sum += texture2D(openfl_Texture, vTexCoord - step2) * 0.05399;
            sum += texture2D(openfl_Texture, vTexCoord + step2) * 0.05399;
            sum += texture2D(openfl_Texture, vTexCoord - vRadiusTexel) * 0.00443;
            sum += texture2D(openfl_Texture, vTexCoord + vRadiusTexel) * 0.00443;
    ...
    @:glVertexSource("attribute vec4 openfl_Position;
        ...
        void main(void) {
            ...
            vRadiusTexel = uRadius / uTextureSize;
        }")
    public function new()
    {
        super();
